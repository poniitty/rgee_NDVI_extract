...
source("scripts/FUNCTION_extract_Landsat_NDVI_gee.R")
...
drive_folder <- paste0("TEMP_Landsat_",iAREA)
...
tifs <- list.files(area_dl_dir, pattern = "GMT.tif$")

image_df <- tibble(file = tifs)
image_df$date <- ymd(unlist(lapply(image_df$file, function(x) str_split(x, "_")[[1]][6])))
...
# First round of masking and selection
...
image_df$collection <- unlist(lapply(image_df$file, function(x) str_split(x, "_")[[1]][2]))
image_df$tier <- unlist(lapply(image_df$file, function(x) str_split(x, "_")[[1]][3]))
image_df$satid <- unlist(lapply(image_df$file, function(x) str_split(x, "_")[[1]][4]))
image_df$path <- as.numeric(substr(unlist(lapply(image_df$file, function(x) str_split(x, "_")[[1]][5])),1,3))
image_df$row <- as.numeric(substr(unlist(lapply(image_df$file, function(x) str_split(x, "_")[[1]][5])),4,6))
image_df$date <- ymd(unlist(lapply(image_df$file, function(x) str_split(x, "_")[[1]][6])))
image_df$time <- gsub(".tif","",unlist(lapply(image_df$file, function(x) str_split(x, "_")[[1]][7])))
